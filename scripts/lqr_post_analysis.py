...
num_successes = {}
for suffix in ("stdev0.0","lqr"):
    path = os.path.join(os.path.expanduser("~"), "Downloads", "poppy_walk_data", f"2026_03_02_{suffix}")
...
U1, p_val = mannwhitneyu(num_successes["stdev0.0"], num_successes["lqr"], method="asymptotic", alternative="less")
print(f"Mann-Whitney U test (asymptotic): {U1=}, {p_val=}")

U1, p_val = mannwhitneyu(num_successes["stdev0.0"], num_successes["lqr"], method="exact", alternative="less")
print(f"Mann-Whitney U test (exact, does not correct for ties): {U1=}, {p_val=}")


data = [num_successes["stdev0.0"], num_successes["lqr"]]
